...
const ToshinItemsModal: React.FC<Props> = ({ show, onClose, services }) => {
  // 修正：將 useMemo 移至元件頂層，符合 Hooks 規則
  const grouped = useMemo(() => {
    return services.reduce<Record<string, Record<string, ToshinService[]>>>((acc, item) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 no-print">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
        <div className="sticky top-0 bg-white p-4 border-b flex justify-between items-center">
          ...
          <button className="text-gray-500 hover:text-gray-800" onClick={onClose}><X size={24} /></button>
        </div>
        <div className="p-6">
